...
Eigen::MatrixXi Delaunay_KNN(Eigen::MatrixXd knn_locations_including_self, Eigen::MatrixXi idx){   
    ...
    n = svd.matrixU()(Eigen::indexing::all, svd.matrixU().cols()-1);
    n_plane = {n(0), n(1), n(2)};
    ...
    Eigen::MatrixXi adjacent_triangles_idx_x = idx(Eigen::indexing::all,adjacent_triangles_idx_local(Eigen::indexing::all,0));
    Eigen::MatrixXi adjacent_triangles_idx_y = idx(Eigen::indexing::all,adjacent_triangles_idx_local(Eigen::indexing::all,1));
    Eigen::MatrixXi adjacent_triangles_idx_z = idx(Eigen::indexing::all,adjacent_triangles_idx_local(Eigen::indexing::all,2));
...
double PerTriangleLaplacianTriangleFanCurvature(Eigen::MatrixXi adjacent_triangles_idx, const Eigen::MatrixXd& V, const Eigen::MatrixXd& N){
    ...
    for (int i = 0; i < N_triangles; i++){    
      ...
      n_triangle_face = N(triangle_verts, Eigen::indexing::all); // 3, 3
      Eigen::MatrixXd AB_mat = triangle(0, Eigen::indexing::all) - triangle(1, Eigen::indexing::all);
      Eigen::MatrixXd AC_mat = triangle(0, Eigen::indexing::all) - triangle(2, Eigen::indexing::all);
